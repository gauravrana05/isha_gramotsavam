...
export default function EditEventPage() {
  ...
  const calculateRegistrationStatus = useCallback(() => {
    ...
    if (!regStart && !regEnd) {
      return formData.isRegistrationOpen;
    }
    ...
    try {
      ...
      if (eventSnapshot.exists()) {
        ...
        setFormData({
          ...
          startDate: eventData.startDate?.toDate ? eventData.startDate.toDate().toISOString().split('T')[0] : '',
          endDate: eventData.endDate?.toDate ? eventData.endDate.toDate().toISOString().split('T')[0] : '',
          registrationStartDate: eventData.registrationStartDate?.toDate ? eventData.registrationStartDate.toDate().toISOString().split('T')[0] : '',
          registrationEndDate: eventData.registrationEndDate?.toDate ? eventData.registrationEndDate.toDate().toISOString().split('T')[0] : '',
          sports: eventData.sports || [],
          venues: eventData.venues || [],
          maxTeams: eventData.maxTeams || 16,
