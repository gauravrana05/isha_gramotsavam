// src/app/[lang]/public/sports/[sportId]/rules/page.tsx
'use client'
...
const formatCategory = (sport: any): string => {
  ...
  if (categories.length === 1) {
    ...
    return '(Men\'s, Women\'s & Mixed)'
  }
  
  return '(Category TBD)'
}

// Helper function to get sport-specific team composition
const getTeamComposition = (sport: any) => {
  const name = sport.name.toLowerCase()
  if (name.includes('volleyball')) {
    return `In one team, there should be a minimum of 6 players and 1 substitute (6+1), and a maximum of 6 players and 6 substitutes (6+6). One player can only participate in one team.`
  }
...
const getTimingRequirement = (sport: any) => {
  const name = sport.name.toLowerCase()
  if (name.includes('volleyball')) {
    return 'The main 6 players should arrive at the match venue and ensure their presence 30 minutes before the commencement of the match.'
  }
  if (name.includes('throwball')) {
    ...
    return (
      <main className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-[#4A2F1D]">Loading rules...</p>
        </div>
      </main>
    )
  }
        ...
        <Container>
          <div className="text-center">
            <h1 className="text-3xl md:text-5xl lg:text-5xl font-semibold text-white mb-4 font-fira">
              Isha Gramotsavam {sport.name} {formatCategory(sport)} Rules
            </h1>
          </div>
        </Container>
      </section>
      <div className="">
        ...
        <section className="py-4">
          <Container>
            ...
            <nav className="text-black text-base font-fira font-semibold">
              <Link href={`/${lang}/public`} className="hover:text-[#F28C38] transition-colors">Home</Link>
              <span className="mx-2 text-gray-400">/</span>
              <Link href={`/${lang}/public/sports`} className="hover:text-[#F28C38] transition-colors">Sports</Link>
              <span className="mx-2 text-gray-400">/</span>
              <Link href={`/${lang}/public/sports/${sportId}`} className="hover:text-[#F28C38] transition-colors">{sport.name}</Link>
              <span className="mx-2 text-gray-400">/</span>
              <span className="text-[#F28C38] font-semibold">Rules</span>
            </nav>
          </Container>
        </section>
            ...
            <div className="max-w-4xl mx-auto">
              <div className="prose prose-lg max-w-none">
                ...
                <ol className="space-y-6 text-[#4A2F1D] font-fira">
                  ...
                  <li>
                    <p className="leading-relaxed">
                      <strong>{getTimingRequirement(sport)}</strong>
                    </p>
                  </li>
