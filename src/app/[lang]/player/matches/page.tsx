...
export default function PlayerMatchesPage() {
  ...
  // Fetch matches for player's teams
  const { 
    data: matchesData, 
        ...
        <div className="text-sm">
          <div className="flex items-center justify-between">
            <span className={`font-medium ${
              match.isMyTeam1 ? 'text-[#F28C38]' : 
              match.winnerName === match.team1Name ? 'text-green-600' : 'text-gray-900'
            }`}>
              {match.team1Name}
              {match.isMyTeam1 && <span className="ml-1 text-xs">(You)</span>}
            </span>
          ...
          <div className="flex items-center justify-between">
            <span className={`font-medium ${
              match.isMyTeam2 ? 'text-[#F28C38]' : 
              match.winnerName === match.team2Name ? 'text-green-600' : 'text-gray-900'
            }`}>
              {match.team2Name}
              {match.isMyTeam2 && <span className="ml-1 text-xs">(You)</span>}
            </span>
    ...
    },
    {
      ...
      render: (_, match) => {
        if (match.status === 'completed') {
          if (match.winnerName) {
            const isWinner = match.winnerName === match.myTeamName;
            return (
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-full">
      ...
      <AdvancedTable<MatchRow>
        ...
        actions={[
          {
            ...
            icon: Eye,
            onClick: (match) => {
              router.push(`/${lang}/player/matches/${match.id}`);
            },
            variant: 'secondary'
          }
        ]}
        ...
        emptyState={{
          ...
          description: 'Your teams haven\'t played any matches yet.',
          action: {
            label: 'View Fixtures',
            onClick: () => router.push(`/${lang}/player/fixtures`)
          }
        }}
