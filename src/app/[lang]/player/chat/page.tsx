...
export default function PlayerChatPage() {
  ...
  // Get player's team venue (auto-detect since player has only one team)
  const { data: playerTeam } = api.teams.getPlayerTeam.useQuery(
    undefined,
    { enabled: !!user }
  );

  const venueId = playerTeam?.venueId;
